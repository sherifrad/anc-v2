...
const UI = (() => {
  ...
  function collectScans() {
    return Array.from(document.querySelectorAll('#ultraBody .scan-row')).map(tr => ({
      ...
      findings: tr.querySelector('.scan-findings')?.value || '',
      recs:     tr.querySelector('.scan-recs')?.value   || '',
      biometrics:{
        BPD:           tr.querySelector('.bio-bpd')?.value       || '',
        HC:            tr.querySelector('.bio-hc')?.value        || '',
        AC:            tr.querySelector('.bio-ac')?.value        || '',
        FL:            tr.querySelector('.bio-fl')?.value        || '',
        AFI:           tr.querySelector('.bio-afi')?.value       || '',
        DVP:           tr.querySelector('.bio-dvp')?.value       || '',
        EFW:           tr.querySelector('.bio-efw')?.value       || '',
        placentaLocation: tr.querySelector('.bio-placenta')?.value || '',
        placentaOS:    tr.querySelector('.bio-placenta-os')?.value || '',
      },
      doppler:{
        UA_PI:  tr.querySelector('.dop-ua')?.value  || '',
        MCA_PI: tr.querySelector('.dop-mca')?.value || '',
        DV_PI:  tr.querySelector('.dop-dv')?.value  || '',
        UtA_PI: tr.querySelector('.dop-uta')?.value || '',
      },
...
const UI = (() => {
  ...
  function scanRowHTML(scan={}, idx, lmpDate) {
    ...
    <tr class="scan-detail-row" data-parent="${idx}">
      <td colspan="5" style="padding:0 8px 10px 12px;background:#f8fbff">
        <div class="us-subfields" style="margin-top:6px">
          ...
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:8px;margin-top:4px">
            <button class="btn-chart ${canChart?'':'btn-chart-disabled'}" data-idx="${idx}" ${canChart?'':'disabled'} title="${canChart?'View growth charts':'Enter BPD, HC, AC, FL to enable charts'}">
              📈 Growth Charts${canChart?'':' (need biometry)'}
            </button>
            ${hasDoppler?`<button class="btn-chart btn-doppler-chart" data-idx="${idx}">📊 Doppler Charts</button>`:''}
          </div>
          ...
          <div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border)">
            <div style="font-size:10px;font-weight:700;color:var(--tx-mid);text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px">📝 Findings / Clinical Impression</div>
            <textarea class="scan-findings" placeholder="General findings, fetal presentation, placental assessment, clinical impression..." style="min-height:60px;width:100%;font-size:12px">${scan.findings||''}</textarea>
          </div>

          <div style="margin-top:8px;margin-bottom:6px">
            <div style="font-size:10px;font-weight:700;color:var(--tx-mid);text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px">📋 Recommendations</div>
            <textarea class="scan-recs" placeholder="Scan recommendations, follow-up plan..." style="min-height:36px;width:100%;font-size:12px">${scan.recs||''}</textarea>
          </div>
  ...
  function collectScans() {
    return Array.from(document.querySelectorAll('#ultraBody .scan-row')).map(tr => {
      ...
      return {
        ...
        ga:       tr.querySelector('.scan-ga-display')?.textContent?.replace(/[^0-9+w d]/g,'') || '',
        operator: tr.querySelector('.scan-operator')?.value || '',
        findings: detailRow?.querySelector('.scan-findings')?.value || '',
